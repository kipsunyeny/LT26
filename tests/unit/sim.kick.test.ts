...
describe('power, scatter and timing maths', () => {
  ...
  it('timing: full power within ±80 ms, then linear to ×0.75 and +3° at 400 ms', () => {
    expect(timingPenalty(0)).toEqual({ powerMult: 1, sigmaDeg: 0 });
    expect(timingPenalty(-80)).toEqual({ powerMult: 1, sigmaDeg: 0 });
    const mid = timingPenalty(240);
    expect(mid.powerMult).toBeCloseTo(0.875, 9);
    expect(mid.sigmaDeg).toBeCloseTo(1.5, 9);
    expect(timingPenalty(-400).powerMult).toBeCloseTo(0.75, 9);
...
describe('scatter and timing in play', () => {
  ...
  it('penalty run-up: on time = full power, 240 ms late = ×0.875 power and 1.5° timing σ', () => {
